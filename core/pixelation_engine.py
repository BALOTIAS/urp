"""
Core pixelation engine for the Unofficial Retro Patch.
"""
...
class PixelationEngine:
    """Handles pixelation and image processing operations."""
    ...
    def pixelate_image(image: Image.Image, resize_amount: float) -> Image.Image:
        ...
        """
        if resize_amount <= 0 or resize_amount > 1:
            raise ValueError("resize_amount must be between 0 and 1")
    ...
    def process_image(
        ...
        # Apply pixelation
        pixelated = PixelationEngine.pixelate_image(image, resize_amount)
        ...
        # corrected = PixelationEngine.apply_offset_correction(pixelated, resize_amount)
        ...
        if mask_file and os.path.exists(mask_file):
            ...
            print(f"[UNOFFICIAL RETRO PATCH] Pixelates {file_name} with mask...")
        else:
        ...
        # If black shadows are enabled, we want to have the pixelated shadow areas
        if black_shadows:
...
def pixelate_image(image: Image.Image, resize_amount: float) -> Image.Image:
    """Backward compatibility function for pixelate_image."""
    return PixelationEngine.pixelate_image(image, resize_amount)


def apply_black_shadows(image: Image.Image, shadow_color: Tuple[int, int, int, int] = (0, 0, 0, 255)) -> Image.Image:
    """Backward compatibility function for apply_black_shadows."""
    return PixelationEngine.apply_black_shadows(image, shadow_color)
...
def process_image(
    ...
    """Backward compatibility function for process_image."""
    return PixelationEngine.process_image(image, resize_amount, mask_file, asset_name, black_shadows)